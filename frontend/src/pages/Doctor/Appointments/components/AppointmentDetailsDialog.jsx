    ...
    try {
      const response = await fetch(`${config.API_URL}/queue`, {
        ...
        body: JSON.stringify({
          ...
          assigned_to: doctorData.id,
        }),
      });

      if (response.ok) {
        console.log("Appointment added to the queue");
        fetchAppointments();
      } else {
        console.error("Error adding appointment to queue");
      }
    } catch (error) {
  ...
  const handleReject = async () => {
    try {
      const response = await fetch(
        `${config.API_URL}/appointments/${appointment.appointment_id}/reject`,
        {
          method: "PUT", // Use PUT to match the server's expected method
          headers: {
            "Content-Type": "application/json",
          },
          body: JSON.stringify({
            status: "rejected", // Status will be updated as "rejected"
          }),
        }
      );
      ...
      if (response.ok) {
        ...
        fetchAppointments(); // Re-fetch appointments after rejection
      } else {
        console.error("Error rejecting the appointment");
      }
    } catch (error) {
      console.error("Error:", error);
    }

    handleCloseDialog(); // Close the dialog after handling rejection
  };
            ...
            <Box mt={2}>
              ...
              "immunization" ? (
                <>
                  ...
                  <Typography>
                    Birthdate:{" "}
                    {moment(additionalInfo.birthdate).format("MMMM D, YYYY")}
                  </Typography>
                  <Typography>
